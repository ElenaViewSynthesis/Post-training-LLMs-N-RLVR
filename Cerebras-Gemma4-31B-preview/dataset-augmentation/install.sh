...
echo "    Upload: hf buckets sync ./data hf://buckets/borntobeignored/OpenThoughts-Agents-SFT-250k"
echo "    Download: hf buckets sync hf://buckets/borntobeignored/OpenThoughts-Agents-SFT-250k ./local"
echo ""
echo "=== Then run stages in order ==="
echo "    python3 stream_refinement_worker.py --status-only"
echo "    python3 stream_refinement_worker.py --limit 1 --no-sync"
echo "    python3 stream_refinement_worker.py"
echo "    python3 augment_150k_rows.py --dry-run --no-sync"
echo "    python3 augment_150k_rows.py --no-sync"
